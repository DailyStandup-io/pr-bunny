...
- Mock data and demo: a fictional world in `src/mock/` (fixtures, test factories, fake `gh`/`claude`/`codex` in `src/mock/bin`) and `bun run demo` (port 4478, throwaway folder, never touches ~/.pr-bunny or real GitHub); see PLAN.md › Mock data and demo. Keep the fakes in step when the app's `gh`/agent command lines change.
